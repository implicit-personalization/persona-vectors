...
def extract_activations(
    ...
    for attempt in range(max_retries):
        try:
            return _extract_single_trace(
                model, input_ids_list, masks, remote, on_status
            )
...
def _extract_chunked(
    ...
    but bounds peak memory so long biographies fit. Cross-sample mean is
    computed inside the session so only the final ``(num_layers, hidden)``
            ...
            for start_layer in range(0, model.num_layers, chunk_size):
                ...
                with model.trace(ids.unsqueeze(0)) as tracer:
                    ...
                    for layer_idx in range(start_layer, end_layer + 1):
                        layer_out = model.layers_output[layer_idx][0]
                        layer_mean = layer_out[mask.to(device=layer_out.device)].mean(
                            dim=0
                        )
                        saved_hs.append(layer_mean.detach().cpu())
