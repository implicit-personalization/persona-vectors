...
@app.cell
def _(LanguageModel, nnsight, torch):
    ...
    return MODEL_NAME, REMOTE, model, tokenizer
...
def _(
    ...
    def generate_response(
        ...
        token_mask = (
            torch.arange(len(tokenizer(full_text).input_ids)) >= response_start_idx
        )

        return full_text, token_mask

    return ACTIVATIONS_DIR, generate_response, persona
...
def _(
    ...
    return long_hidden_states, short_hidden_states
...
@app.cell
def _(long_hidden_states, persona, plot_layer_similarity, short_hidden_states):
